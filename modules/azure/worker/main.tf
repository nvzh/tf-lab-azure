resource "azurerm_public_ip" "cso_worker_pub_ip" {
  name                = "${var.name}-${var.caseNo}-worker-instance-public-ip-${count.index}"
  count               = var.worker_count
  location            = var.location
  resource_group_name = var.rg
  allocation_method   = "Static"
  domain_name_label   = "${var.name}-${var.caseNo}-worker-${count.index}"
...
resource "azurerm_network_interface" "cso_worker_interface" {
  name                = "${var.name}-${var.caseNo}-worker-net-interface-${count.index}"
  count               = var.worker_count
  location            = var.location
  resource_group_name = var.rg
...
resource "azurerm_virtual_machine" "cso_worker_vm" {
  ...
  name                  = "${var.name}-${var.caseNo}-worker-${count.index}"
  count                 = var.worker_count
  location              = var.location
  resource_group_name   = var.rg
