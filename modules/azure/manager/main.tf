resource "azurerm_public_ip" "cso_manager_pub_ip" {
  name                = "${var.name}-${var.caseNo}-manager-public-ip-${count.index}"
  count               = var.manager_count
  location            = var.location
  resource_group_name = var.rg
  allocation_method   = "Static"
  domain_name_label   = "${var.name}-${var.caseNo}-manager-${count.index}"
...
resource "azurerm_network_interface" "cso_manager_interface" {
  name                = "${var.name}-${var.caseNo}-net-manager-${count.index}"
  count               = var.manager_count
  location            = var.location
  resource_group_name = var.rg
...
resource "azurerm_virtual_machine" "cso_manager_vm" {
  ...
  name                  = "${var.name}-${var.caseNo}-manager-${count.index}"
  count                 = var.manager_count
  location              = var.location
  resource_group_name   = var.rg
