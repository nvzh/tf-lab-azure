resource "azurerm_public_ip" "cso_win_pub_ip" {
  name                = "${var.name}-${var.caseNo}-win-instance-public-ip-${count.index}"
  count               = var.win_worker_count
  location            = var.location
  resource_group_name = var.rg
  allocation_method   = "Dynamic"
  domain_name_label   = "${var.name}-${var.caseNo}-win-${count.index}"
...
resource "azurerm_network_interface" "cso_win_interface" {
  name                = "${var.name}-${var.caseNo}-win-net-interface-${count.index}"
  count               = var.win_worker_count
  location            = var.location
  resource_group_name = var.rg
...
resource "azurerm_virtual_machine" "cso_win_vm" {
    ...
    name                  = "${var.name}-${var.caseNo}-win-${count.index}"
    count                 = var.win_worker_count
    location              = var.location
    resource_group_name   = var.rg
